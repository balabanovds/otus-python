...
def parse_log_file(log_file: str) -> StatData:
    ...
    logger.info(f'in file {log_file} found {len(data)} records')
...
def _add_data(sd: StatData, url: str, duration: str):
    ...
    logger.debug(f'processing url \'{url}\' with duration {duration}')
    ...
    try:
        ...
        logger.exception(f'duration {duration} is not valid')
        sd.counters['err'] += 1
        return
    sd.counters['all'] += 1
...
def create_report(to_file: str, data: Type[dict], max_records: int):
    ...
    try:
        ...
        sorted_list = sorted(list(data.values()), key=lambda i: i['time_sum'], reverse=True)

        s = t.safe_substitute(table_json=sorted_list[:max_records])
        fd = open(to_file, 'w')
        fd.write(s)
        fd.close()
        logger.info(f'created report file: {to_file}')
    except:
        logger.exception('error occured while creating report')
