...
def parse_log_file(log_file: str, logger: logging.Logger) -> dict:
    logger.info(f'processing file: {log_file}')
    (_, file_encoding) = mimetypes.guess_type(log_file)
    fd = gzip.open(log_file, 'rt') if file_encoding == 'gzip' else open(log_file, 'r')
